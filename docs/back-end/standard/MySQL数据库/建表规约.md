...
9. 【强制】表必备三字段：id, create_date, edit_date，两个可选字段create_user，edit_user。 
<br><span style="color:orange">说明</span>：其中id必为主键，类型为varchar(32)，使用雪花算法（推荐）或uuid。create_date, edit_date的类型均为datetime类型，前者现在时表示主动创建，后者过去分词表示被动更新。 
